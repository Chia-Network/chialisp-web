...
This tutorial teaches you how to spend a coin with any puzzle using RPC calls. We're using the password-locked coin puzzle from [How to lock coin with a custom puzzle](custom_puzzle_lock) tutorial in this tutorial as well.
...
When you search for receive address, you'll see all coins locked by this puzzle. Select the one you want to spend. That will get you to coin's detail to get the amount, puzzle hash, and parent info.
...
Same command we used for getting puzzle's hash in [How to lock coin with a custom puzzle](custom_puzzle_lock#get-puzzle-hash-from-a-puzzle) (`opc -H <compiled_puzzle>`) can be used for getting serialized version of our puzzle and solution as well. Serialized version will be included in response **on the second line**.
